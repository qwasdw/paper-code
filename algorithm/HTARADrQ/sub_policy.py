...
class SubAgent(object):
    ...
    def save(self, index):
        torch.save(self.actor.state_dict(), "sub_actor_" + str(index))
        torch.save(self.critic.state_dict(), "sub_critic_" + str(index))

    def load(self, index):
        self.actor.load_state_dict(torch.load("actor_" + str(index)))
